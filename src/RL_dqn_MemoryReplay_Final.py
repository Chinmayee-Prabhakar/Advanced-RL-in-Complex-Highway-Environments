...
# This is the python file that uses Replay Memory (RM) in PyTorch, 
...
# Create a class to use a memory buffer replay.
# The RM stores the transitions that the agent observes and then
# reuses this data later.
# This method is used to calculate loss on batches with no associated states.
...
def training_function():
    ...
    for i_episode in range(num_episodes):
        ...
        print(f'episode: {i_episode} | score: {reward} | average score: {avg_score} ')

        #if i_episode % 20 == 0:
        print(f"Mean episode {i_episode}/{num_episodes} reward is:{total_reward:.2f}")
        total_reward = 0
...
if __name__ == "__main__":
    ...
    torch.save(policy_net.state_dict(), "../models/model_1001")

    # In case the training_function is already trained, we load it
    steps_done = 1000000000000
    policy_net.load_state_dict(torch.load("../models/model_1001"))
